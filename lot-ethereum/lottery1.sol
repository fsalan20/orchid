...
contract OrchidLottery1 {
    ...
    function move(address funder, address signer, IERC20 token, uint256 amount, uint256 recover, uint256 transfer, uint256 retrieve) external {
        require(token.transferFrom(funder, address(this), amount));
        move_(funder, signer, token, amount, recover, transfer, retrieve);
    }
